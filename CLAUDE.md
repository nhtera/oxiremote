...
- `audio` feature on `crates/desktop` subcrate — **not in default**; enable with `cargo build -p desktop --features desktop/audio`. Pulls `audiopus` (vendored libopus) + `rubato` resampler. Phase-02 scaffold only: `AudioCapture` trait + per-platform stubs compile and unit-test, but no capture path is operational yet (Windows WASAPI gated behind phase-02 step-1 kill-switch).
