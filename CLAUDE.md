...
- Per-session override: the SPA can append `?force_pipeline=jpeg|h264|vp9|av1|auto` to the WS upgrade. The agent parses + validates and uses it for **that session only** without touching the env-var preference. Used by the SPA to honor `FallbackPending` (3 s no-IDR watchdog).
