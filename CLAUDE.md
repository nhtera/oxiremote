...
- `hevc` feature — **NOT in `default = ["desktop","h264"]`**. Opt-in: `cargo build --features hevc`. macOS-only (VideoToolbox H.265 Main L5.1); CI release workflow does not yet add it (Phase 06 pending). `pipeline_selection::choose()` under `Auto` prefers HEVC > H.264 > JPEG when this feature is compiled. Wire codec value: `"h265-main-5.0"`; `SignalOut::Pipeline.mode = "h265"`; RTP PT 99; `hvcc_description_b64` carries the ISO 14496-15 HEVCDecoderConfigurationRecord. Fails-closed on incapable clients.
- Build without remote desktop: `cargo build --no-default-features` (e.g. headless CI).
- Per-session override: the SPA can append `?force_pipeline=jpeg|h264|hevc|auto` to the WS upgrade. The agent parses + validates and uses it for **that session only** without touching the env-var preference. Used by the SPA to honor `FallbackPending` (3 s no-IDR watchdog).
...
| `OXI_VIDEO_PIPELINE=auto\|h264\|jpeg\|hevc` | Operator preference for remote-desktop pipeline. **Default `auto`** post phase-01: resolves HEVC > H.264 > JPEG based on compiled features + client capability. `h264` / `hevc` fail-closed on incapable clients (no silent fallback); `jpeg` forces JPEG. Unknown values fall back to `auto`. `hevc` requires `--features hevc` (NOT in default build). |
